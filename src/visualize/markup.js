...
var _activate = function(el) {
  ...
  return Array.from(codes).map((code) =>
    (function() {
      ...
      return popup.parentNode.addEventListener('click', event =>
        popup.style.display = {
          block: 'none',
          none:  'block'
...
const _order = function(data) {
  const nodeMap = {};
  const linkMap = {};
  for (var node of Array.from(data.nodes)) {
    nodeMap[node.id] = node;
  }

  for (var link of Array.from(data.links)) {
...
`;
    ...
    for (var outlet of Array.from(node.inputs)) { addOutlet(outlet, 'in'); }
...
const merge = function(markup) {
  if (markup.length !== 1) {
    const div = document.createElement('div');
    for (var el of Array.from(markup)) { div.appendChild(wrap(el)); }
    div.update = () => (() => {
      const result = [];
